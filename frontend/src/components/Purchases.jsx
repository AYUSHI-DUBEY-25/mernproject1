...
function Purchases() {
  ...
  const [errorMessage, setErrorMessage] = useState(""); // Initially empty, to handle errors correctly
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // Sidebar open state

  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem("user"));
  const token = user?.token; // Using optional chaining
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      try {
        ...
        setPurchases(response.data.courseData || []); // Handle case where no courseData is returned
      } catch (error) {
