...
function Signup() {
  ...
  return <div className='bg-gradient-to-r from-black to-gray-800 min-h-screen flex items-center justify-center px-4'> 
  <div className="container mx-auto flex flex-col items-center text-white w-full max-w-lg"> 
    {/*Header*/}
    <header className="w-full flex justify-between items-center p-5"> 
      <div className="flex items-center space-x-2">
        <img src='logo.webp' className="w-16 h-16 rounded-full"></img>
        <Link to={"/"} className="text-xl text-pink-600 font-bold font-serif">HustleLearn</Link>
      </div>
      <div className="flex space-x-4">
        <Link to={"/login"} className="text-white py-2 px-4 border border-white rounded text-sm">Login</Link>
        <Link to={"/courses"} className="text-white py-2 px-4 border border-white rounded text-sm">Join Now</Link>
      </div>
    </header>
    {/* Signup Form */}
    <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-full">
      ...
      <p className="text-center text-gray-400 mb-4">Signup to join!</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <input type="text" id="firstname" value={firstName} onChange={(e) => setFirstName(e.target.value)} 
          className="w-full p-3 rounded-md bg-gray-800 border border-gray-700 focus:ring-2 focus:ring-blue-500" placeholder="First Name" />
        <input type="text" id="lastname" value={lastName} onChange={(e) => setLastName(e.target.value)} 
          className="w-full p-3 rounded-md bg-gray-800 border border-gray-700 focus:ring-2 focus:ring-blue-500" placeholder="Last Name" />
        <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} 
          className="w-full p-3 rounded-md bg-gray-800 border border-gray-700 focus:ring-2 focus:ring-blue-500" placeholder="Email" />
        <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} 
          className="w-full p-3 rounded-md bg-gray-800 border border-gray-700 focus:ring-2 focus:ring-blue-500" placeholder="Password" />
